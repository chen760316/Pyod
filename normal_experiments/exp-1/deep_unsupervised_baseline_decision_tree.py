...
def run(file_path):
    ...
    # 加噪训练样本中被decision treeM模型错误分类的样本
    wrong_classified_train_indices_noise = np.where(y_train != train_label_pred_noise)[0]
    print("加噪训练样本中被decision tree模型错误分类的样本占总加噪训练样本的比例：",
    ...
    print("加噪测试样本中被decision tree模型错误分类的样本占总测试样本的比例：",
    ...
    #
    # dt_repair = tree.DecisionTreeClassifier(class_weight='balance')
    # dt_repair.fit(X_train_copy_repair, y_train_copy_repair)
    # y_train_pred = dt_repair.predict(X_train_copy_repair)
    # y_test_pred = dt_repair.predict(X_test_copy_repair)
    #
    # print("*" * 100)
    # # 训练样本中被decision tree模型错误分类的样本
