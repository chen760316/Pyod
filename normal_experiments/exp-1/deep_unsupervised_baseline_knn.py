...
def run(file_path):
    ...
    train_label_pred = knn_model.predict(X_train)
    test_label_pred = knn_model.predict(X_test)
    ...
    print("*" * 100)
    knn_model_noise = KNeighborsClassifier(n_neighbors=22)
    knn_model_noise.fit(X_train_copy, y_train)
    train_label_pred_noise = knn_model_noise.predict(X_train_copy)
    test_label_pred_noise = knn_model_noise.predict(X_test_copy)

    # 加噪训练样本中被KNN模型错误分类的样本
    wrong_classified_train_indices_noise = np.where(y_train != train_label_pred_noise)[0]
    print("加噪训练样本中被KNN模型错误分类的样本占总加噪训练样本的比例：",
    ...
    print("加噪测试样本中被KNN模型错误分类的样本占总测试样本的比例：",
    ...
    # choice 利用损失函数
    # 在加噪数据集D'上训练的KNN模型，其分类错误的样本在原含噪数据D'中的索引
