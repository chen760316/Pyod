...
dt_model.fit(X_train_copy, y_train)
...
decision_values = dt_model.predict_proba(X_copy)
predicted_labels = np.argmax(decision_values, axis=1)
# 计算每个样本的hinge损失
num_samples = X_copy.shape[0]
num_classes = dt_model.classes_.shape[0]
...
train_label_pred = dt_clf.predict(X_train)
test_label_pred = dt_clf.predict(X_test)
...
print("*" * 100)
train_label_pred_noise = dt_model.predict(X_train_copy)
test_label_pred_noise = dt_model.predict(X_test_copy)
