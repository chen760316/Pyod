...
def run(file_path):
    ...
    test_label_pred = nb_model.predict(X_test)
    ...
    print("完整数据集D中被naive bayes模型错误分类的样本占总完整数据的比例：",
    ...
    print("加噪测试样本中被naive bayes模型错误分类的样本占总测试样本的比例：",
    ...
    r_acc = str(accuracy_score(y_test, y_test_pred))
    ...
    r_precision = str(precision_score(y_test, y_test_pred, average='weighted'))
    print("分类器在修复后的加噪测试集中的分类召回率：" + str(recall_score(y_test, y_test_pred, average='weighted')))
    r_recall = str(recall_score(y_test, y_test_pred, average='weighted'))
    print("分类器在修复后的加噪测试集中的分类F1分数：" + str(f1_score(y_test, y_test_pred, average='weighted')))
    r_f1 = str(f1_score(y_test, y_test_pred, average='weighted'))
    return r_acc, r_f1, r_precision, r_recall


if __name__ == '__main__':
    paths = [
        "../datasets/multi_class/drybean.xlsx",
        ...
        "../datasets/multi_class/body/body.csv",
        # "../datasets/multi_class/financial/financial.csv",
    ...
    for file_path in paths:
        acc, f1, precision, recall = run(file_path)
        res_list[0].append(acc)
        res_list[1].append(f1)
        res_list[2].append(precision)
        res_list[3].append(recall)
    for res in res_list:
        print(" ".join(res))