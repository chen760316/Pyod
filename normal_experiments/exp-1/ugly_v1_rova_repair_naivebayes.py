...
decision_values = nb_model.predict_proba(X_copy)
predicted_labels = np.argmax(decision_values, axis=1)
# 计算每个样本的hinge损失
num_samples = X_copy.shape[0]
num_classes = nb_model.classes_.shape[0]
...
train_label_pred = nb_clf.predict(X_train)
test_label_pred = nb_clf.predict(X_test)
...
print("*" * 100)
train_label_pred_noise = nb_model.predict(X_train_copy)
test_label_pred_noise = nb_model.predict(X_test_copy)
