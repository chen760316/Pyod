...
# 训练样本中被KNN模型错误分类的样本
wrong_classified_train_indices = np.where(y_train != knn_model.predict(X_train))[0]
print("训练样本中被KNN模型错误分类的样本占总训练样本的比例：", len(wrong_classified_train_indices)/len(y_train))

# 测试样本中被KNN模型错误分类的样本
wrong_classified_test_indices = np.where(y_test != knn_model.predict(X_test))[0]
print("测试样本中被KNN模型错误分类的样本占总测试样本的比例：", len(wrong_classified_test_indices)/len(y_test))

# 整体数据集D中被KNN模型错误分类的样本
print("完整数据集D中被KNN模型错误分类的样本占总完整数据的比例：", (len(wrong_classified_train_indices) + len(wrong_classified_test_indices))/(len(y_train) + len(y_test)))
...
wrong_classified_train_indices_noise = np.where(y_train != knn_model_noise.predict(X_train_copy))[0]
print("加噪训练样本中被KNN模型错误分类的样本占总加噪训练样本的比例：", len(wrong_classified_train_indices_noise)/len(y_train))

# 加噪测试样本中被KNN模型错误分类的样本
wrong_classified_test_indices_noise = np.where(y_test != knn_model_noise.predict(X_test_copy))[0]
print("加噪测试样本中被KNN模型错误分类的样本占总测试样本的比例：", len(wrong_classified_test_indices_noise)/len(y_test))

# 整体加噪数据集D中被KNN模型错误分类的样本
print("完整数据集D中被KNN模型错误分类的样本占总完整数据的比例：", (len(wrong_classified_train_indices_noise) + len(wrong_classified_test_indices_noise))/(len(y_train) + len(y_test)))
