...
测试ugly v2 outliers检测规则的可扩展性（重点关注谓词的可扩展性）
"""
...
with memory_timer():
    ...
    end_time = time.time()  # 结束时间
    print("异常检测器PReNet训练耗时：", end_time - start_time)
    ...
    end_time = time.time()  # 结束时间
    print("异常检测器PReNet在原始测试集预测耗时：", end_time - start_time)
    ...
    end_time = time.time()  # 结束时间
    print("异常检测器PReNet在加噪测试集预测耗时：", end_time - start_time)
    ...
    end_time = time.time()  # 结束时间
    print("random forest模型在原始和加噪数据集中训练和预测耗时：", end_time - start_time)
    ...
    end_time = time.time()  # 结束时间
    print("确定有影响力的特征耗时：", end_time - start_time)
    ...
    end_time = time.time()  # 结束时间
    print("选取活动域过小的特征耗时：", end_time - start_time)
