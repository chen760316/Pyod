...
with memory_timer():
    ...
    # 对训练数据进行处理后重新训练softmax模型

    # SECTION 原始数据中的softmax分类准确度
    # print("*" * 100)
    # print("原始训练集中softmax分类准确度：" + str(accuracy_score(y_train, softmax_model.predict(X_train))))
    # print("原始测试集中softmax分类准确度：" + str(accuracy_score(y_test, softmax_model.predict(X_test))))
    # print("*" * 100)
    ...
    # # 重新训练softmax模型
    # softmax_model_split = LogisticRegression(multi_class='ovr', solver='liblinear', random_state=42, class_weight='balanced')
    # softmax_model_split.fit(X_train_split, y_train_split)
    # print("*" * 100)
    # print("去除异常值中分类错误的样本后的训练集softmax分类准确度：" + str(accuracy_score(y_train_split, softmax_model_split.predict(X_train_split))))
    # print("去除异常值中分类错误的样本后的测试集softmax分类准确度：" + str(accuracy_score(y_test, softmax_model_split.predict(X_test))))
    # print("*" * 100)
    ...
    # softmax_model_h = LogisticRegression(multi_class='ovr', solver='liblinear', random_state=42, class_weight='balanced')
    # softmax_model_h.fit(X_train_h, y_train_h)
    # print("*" * 100)
    # print("去除损失高于阈值的样本后的训练集softmax分类准确度：" + str(accuracy_score(y_train_h, softmax_model_h.predict(X_train_h))))
    # print("去除损失高于阈值的样本后的测试集softmax分类准确度：" + str(accuracy_score(y_test, softmax_model_h.predict(X_test))))
    # print("*" * 100)
    ...
    # softmax_model_o = LogisticRegression(multi_class='ovr', solver='liblinear', random_state=42, class_weight='balanced')
    # softmax_model_o.fit(X_train_o, y_train_o)
    # print("*" * 100)
