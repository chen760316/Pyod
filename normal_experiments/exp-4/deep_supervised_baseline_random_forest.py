...
print("完整数据集D中被random_forest模型错误分类的样本占总完整数据的比例：",
...
print("random_forest模型在加噪测试集中的分类精确度：" + str(precision_score(y_test, y_test_pred, average='weighted')))
print("random_forest模型在加噪测试集中的分类召回率：" + str(recall_score(y_test, y_test_pred, average='weighted')))
print("random_forest模型在加噪测试集中的分类F1分数：" + str(f1_score(y_test, y_test_pred, average='weighted')))

"""ROC-AUC指标"""
y_test_prob = rf_model_noise.predict_proba(X_test)
roc_auc_test = roc_auc_score(y_test, y_test_prob, multi_class='ovr')  # 一对多方式
print("random_forest模型在加噪测试集中的ROC-AUC分数：" + str(roc_auc_test))
...
#     print(f"random_forest模型在修复测试集中的PR AUC 分数（类 {i}）: {pr_auc}")
# # 如果需要计算所有类的宏平均 PR 分数
# macro_pr_score = sum(pr_scores) / len(pr_scores)
# print("random_forest模型在修复测试集中的宏平均AP分数:", macro_pr_score)
...
#     print(f"random_forest模型在修复测试集中的AP分数（类 {i}）: {ap_score}")
#
# # 如果需要计算所有类的宏平均 AP 分数
# macro_ap_score = sum(ap_scores) / len(ap_scores)
# print("random_forest模型在修复测试集中的宏平均AP分数:", macro_ap_score)
...
# choice 利用损失函数
# 在加噪数据集D'上训练的random_forest模型，其分类错误的样本在原含噪数据D'中的索引
...
# subsection 重新在修复后的数据上训练random_forest模型
...
print("加噪标签修复后，训练样本中被random_forest模型错误分类的样本占总训练样本的比例：", len(wrong_classified_train_indices)/len(y_train))

# 测试样本中被random_forest模型错误分类的样本
wrong_classified_test_indices = np.where(y_test != y_test_pred)[0]
print("加噪标签修复后，测试样本中被random_forest模型错误分类的样本占总测试样本的比例：", len(wrong_classified_test_indices)/len(y_test))

# 整体数据集D中被random_forest模型错误分类的样本
print("加噪标签修复后，完整数据集D中被random_forest模型错误分类的样本占总完整数据的比例：",
      (len(wrong_classified_train_indices) + len(wrong_classified_test_indices))/(len(y_train) + len(y_test)))
...
print("random_forest模型在修复测试集中的分类精确度：" + str(precision_score(y_test, y_test_pred, average='weighted')))
print("random_forest模型在修复测试集中的分类召回率：" + str(recall_score(y_test, y_test_pred, average='weighted')))
print("random_forest模型在修复测试集中的分类F1分数：" + str(f1_score(y_test, y_test_pred, average='weighted')))

"""ROC-AUC指标"""
y_test_prob = rf_repair.predict_proba(X_test)
roc_auc_test = roc_auc_score(y_test, y_test_prob, multi_class='ovr')  # 一对多方式
print("random_forest模型在修复测试集中的ROC-AUC分数：" + str(roc_auc_test))
...
# # 计算 PR AUC
# pr_auc = auc(recall, precision)
# print("random_forest模型在修复测试集中的PR AUC 分数:", pr_auc)
#
"""AP指标(不支持多分类)"""
# # 计算预测概率
# y_scores = rf_repair.predict_proba(X_test)
...
# print("random_forest模型在修复测试集中的AP分数:", ap_score)
...
# # 训练样本中被random_forest模型错误分类的样本
# wrong_classified_train_indices = np.where(y_train != y_train_pred)[0]
# print("借助knn修复需要修复的样本后，训练样本中被random_forest模型错误分类的样本占总训练样本的比例：",
#       len(wrong_classified_train_indices)/len(y_train))
#
# # 测试样本中被random_forest模型错误分类的样本
# wrong_classified_test_indices = np.where(y_test != y_test_pred)[0]
# print("借助knn修复需要修复的样本后，测试样本中被random_forest模型错误分类的样本占总测试样本的比例：",
#       len(wrong_classified_test_indices)/len(y_test))
#
# # 整体数据集D中被random_forest模型错误分类的样本
# print("借助knn修复需要修复的样本后，完整数据集D中被random_forest模型错误分类的样本占总完整数据的比例：",
#       (len(wrong_classified_train_indices) + len(wrong_classified_test_indices))
#       /(len(y_train) + len(y_test)))

# section 方案四：将X_copy中训练集和测试集需要修复的元组直接删除，在去除后的训练集上训练random_forest模型
...
# print("删除需要修复的样本后，训练样本中被random_forest模型错误分类的样本占总训练样本的比例：",
...
# print("删除需要修复的样本后，测试样本中被random_forest模型错误分类的样本占总测试样本的比例：",
#       len(wrong_classified_test_indices)/len(y_test_copy_repair))
#
# # 整体数据集D中被random_forest模型错误分类的样本
# print("删除需要修复的样本后，完整数据集D中被random_forest模型错误分类的样本占总完整数据的比例：",
#       (len(wrong_classified_train_indices) + len(wrong_classified_test_indices))
